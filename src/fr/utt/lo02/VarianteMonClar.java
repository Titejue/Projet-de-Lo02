...
public class VarianteMonClar implements Variante {
    ...
    private int numCarteJouable;
    private LinkedList<Carte> cartePourJouer;

    private Carte carteJouable;
    private int sens;
    ...
    // --------------------------- CONSTRUCTEUR ----------------------------------------------------
    public void VarianteMonClar() {
        System.out.println("Vous avez choisi la variante de Monclar");
    }
    ...
    public void carteJouable(Joueur j, LinkedList<Carte> main, LinkedList<Carte> dernieresCartes, int payer) {
        ...
        for (Carte c : main) {
            ...
            // La dernière carte posée était spéciale : As ou neuf
            else {
                if (payer > 0) {
                    ...
                    if (dernieresCartes.getLast().getValeur() == ValeurCarte.AS) {
                        ...
                        // Soit on possède une carte huit
                        else if (ValeurCarte.HUIT == c.getValeur()) {
                            this.carteJouable = c;
                            if (!cartePourJouer.contains(this.carteJouable)) {
                                this.cartePourJouer.add(this.carteJouable);
                            }
                    ...
                    else if (dernieresCartes.getLast().getValeur() == ValeurCarte.NEUF) {
                        if (cartePourJouer.size() > 0) {
                            for (int k = 0; k < cartePourJouer.size(); k++) {
                                cartePourJouer.remove(cartePourJouer.get(k));
                            }
                        }
                        // Soit on possède une carte NEUF
                        if (ValeurCarte.NEUF == c.getValeur()) {
                            this.carteJouable = c;
                            if (!cartePourJouer.contains(this.carteJouable)) {
                                this.cartePourJouer.add(this.carteJouable);
                            }
